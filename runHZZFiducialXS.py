...
def get_linenumber():
    """Get current line number of the program
...
def parseOptions():
    ...
    parser.add_option('',   '--modelNames',dest='MODELNAMES',type='string',default='SM_125',help='Names of models for unfolding, separated by , (comma) . Default is "SM_125"')
...
def processCmd(cmd, lineNumber, quiet = 0):
    ...
    Args:
        cmd (str): command to run on terminal
        lineNumber (int): current line number of python program where it instructs to run the current command
        quiet (int, optional): Run in quite mode without any prints on terminal or show all print of the command. Defaults to 0.

    Raises:
        RuntimeError: Raise a run time error if the command does not succeed.

    Returns:
        str: all outputs of the given command
    """
    output = '\n'
    print("="*51)
...
#Test VM:        cmd = cmd + ' --freezeNuisanceGroups CMS_fakeH_p1_1_8,CMS_fakeH_p1_2_8,CMS_fakeH_p1_3_8,CMS_fakeH_p3_1_8,CMS_fakeH_p3_2_8,CMS_fakeH_p3_3_8 --freezeParameters allConstrainedNuisances'
        ...
        #        combine command generates output with exactly same name.
...
def extractResults(obsName, observableBins, modelName, physicalModel, asimovModelName, asimovPhysicalModel, resultsXS):
    ...
    cmd = 'root -l -b -q "src/addToyDataset.C(\\"xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'.root\\",\\"xs_125.0/'+asimovModelName+'_all_'+obsName+'_13TeV_Asimov_'+asimovPhysicalModel+'.root\\",\\"toy_asimov\\",\\"xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_exp.root\\")"'
    processCmd(cmd, get_linenumber())
    ...
    if (physicalModel=="v3"):
        if (not opt.FIXFRAC):
            if (opt.FIXMASS=="False"):
                cmd =  'combine -n '+obsName+' -M MultiDimFit xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_exp.root -m 125.0 -D toy_asimov '
            else:
                cmd =  'combine -n '+obsName+' -M MultiDimFit xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_exp.root -m '+opt.FIXMASS+' -D toy_asimov --setParameters MH='+opt.FIXMASS
            ...
            output=processCmd(cmd, get_linenumber())
            if (opt.FIXMASS=="False"):
                processCmd('mv higgsCombine'+obsName+'.MultiDimFit.mH125.root xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_result.root', get_linenumber(), 1)
            else:
                processCmd('mv higgsCombine'+obsName+'.MultiDimFit.mH'+opt.FIXMASS.replace('.0.root','.root')+'.root xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_result.root', get_linenumber(), 1)
            cmd = cmd + ' --algo=singles --cl=0.68 --robustFit=1'
            output=processCmd(cmd, get_linenumber())
            ...
            if (opt.FIXMASS=="False"):
                cmd =  'combine -n '+obsName+' -M MultiDimFit xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_exp.root -m 125.0 -D toy_asimov  --setParameters '
            else:
                cmd =  'combine -n '+obsName+' -M MultiDimFit xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_exp.root -m '+opt.FIXMASS+' -D toy_asimov --setParameters MH='+opt.FIXMASS+','
            for obsBin in range(nBins-1):
    ...
    if (physicalModel=="v2"):
        cmd =  'combine -n '+obsName+' -M MultiDimFit xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_exp.root -D toy_asimov --saveWorkspace'
        if (not opt.FIXMASS=="False"):
        ...
        if (opt.FIXMASS=="False"):
            ...
            processCmd('mv higgsCombine'+obsName+'.MultiDimFit.mH'+opt.FIXMASS.rstrip('.0')+'.root '+'xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_result.root', get_linenumber(), 1)
        cmd = cmd + ' --algo=singles --cl=0.68 --robustFit=1'
        output=processCmd(cmd, get_linenumber())
    ...
    if (opt.SYS):
        cmd = cmd.replace('xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_exp.root','-d xs_125.0/'+modelName+'_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_result.root -w w --snapshotName "MultiDimFit"')
